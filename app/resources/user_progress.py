...
# class controlling skills resource
class AdminUserProgress(Resource):
    parser = reqparse.RequestParser()
    parser.add_argument('progress')
    ...
    def get(user, self, id):
        try:
            progress_model = UserProgressModel.find_by_id(id)
            print(progress_model.json())
            return progress_model.json()
        except:
            {'message': 'no progress found'}, 201
...
class UserProgressList(Resource):
    @auth_required(1)
    def get(user, self):
        progress_list = list(map(lambda x: x.json(), UserProgressModel.query.filter_by(user_id=user.id).all()))
        return progress_list


class AdminUserProgressList(Resource):
    @auth_required(3)
    def get(user, self):
        data = list(map(lambda x: x.json(), UserProgressModel.query.all()))
