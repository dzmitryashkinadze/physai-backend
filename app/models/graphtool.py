...
class GraphtoolModel(db.Model):
    """
    This DB model represents a graph.
    """

    __tablename__ = "graphtool"

    # atributes
    id = db.Column(db.Integer, primary_key=True, autoincrement=True)
    graphtool = db.Column(db.Text, nullable=False)
    ...
    def __init__(self, **kwargs):
        super(GraphtoolModel, self).__init__(**kwargs)

    def json(self):
        """Return a JSON representation of a problem."""
        return {
            "id": self.id,
            "graphtool": self.graphtool,
