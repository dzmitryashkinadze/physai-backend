...
class UserProgressTheoryModel(db.Model):
    """
    This DB model represents a user's progress on a problem.
    """

    __tablename__ = "user_progress_theory"

    # atributes
    id = db.Column(db.Integer, primary_key=True, autoincrement=True)
    user_id = db.Column(db.Integer, db.ForeignKey("user.id"), nullable=False)
    problem_id = db.Column(db.Integer, db.ForeignKey("theory.id"), nullable=False)
    ...
    def __init__(self, **kwargs):
        super(UserProgressTheoryModel, self).__init__(**kwargs)
